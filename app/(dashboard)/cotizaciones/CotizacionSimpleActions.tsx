...
export function CotizacionSimpleActions({ id, numero, estado, canDelete = true }: { id: string; numero: string; estado: string; canDelete?: boolean }) {
  ...
  async function handleDuplicar() {
    setLoading(true);
    await fetch(`/api/cotizaciones-simple/${id}/duplicar`, { method: "POST" });
    router.refresh();
    setLoading(false);
  }
  ...
  async function handleDelete() {
    ...
    setLoading(true);
    await fetch(`/api/cotizaciones-simple/${id}`, { method: "DELETE" });
    router.refresh();
    setLoading(false);
  }
  ...
  return (
    <>
      <div className="flex items-center gap-0.5 justify-end">
        ...
        <a
          href={`/api/cotizaciones-simple/${id}/docx`}
          ...
          >
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200">
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">Cotización Simple</span>
            ...
            <div className="flex-1 overflow-hidden rounded-b-2xl bg-gray-50">
              <iframe
                src={`/api/cotizaciones-simple/${id}/preview`}
